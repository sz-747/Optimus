...
/// (<c>cmux hooks &lt;agent&gt; &lt;event&gt;</c>) plus the installer/printer that emits the
...
public static class HooksCommand
{
    ...
    // ---- runtime: `cmux hooks <agent> <event>` -------------------------------------------------
    ...
    private static object ParseRuntime(List<string> args, Func<string, string?> getEnv, string? stdin)
    {
        ...
        // Hooks are gated on the surface id shell integration injects: outside a cmux pane the
    ...
    // ---- installer: `cmux hooks install <agent>` / `cmux hooks print <agent>` ------------------
    ...
    private static object ParseInstall(List<string> args)
    {
        ...
        string baseName = $"cmux-{agent.Name}-hook";
    ...
    /// to the cmux runtime verb and never fails the agent (always exits 0). The CMUX_SURFACE_ID
    /// gate is duplicated here so the snippet is a no-op outside cmux even before cmux.exe runs.
    /// </summary>
    public static string Snippet(AgentDef agent, string format) => format switch
    {
        "ps1" => string.Join("\r\n",
            $"# cmux {agent.Name} hook (generated by `cmux hooks install {agent.Name}`)",
            "param([Parameter(Mandatory = $true)][string]$HookEvent)",
            "if ([string]::IsNullOrEmpty($env:CMUX_SURFACE_ID)) { exit 0 }",
            $"$input | & cmux hooks {agent.Name} $HookEvent",
            "exit 0",
            ""),
        "cmd" => string.Join("\r\n",
            "@echo off",
            $"rem cmux {agent.Name} hook (generated by `cmux hooks install {agent.Name}`)",
            "if \"%CMUX_SURFACE_ID%\"==\"\" exit /b 0",
            $"cmux hooks {agent.Name} %1",
            "exit /b 0",
            ""),

...
- 2026-07-10 · feat/p6-u4-webview2-pane · p6 U4 completion pass · PR #11 · Reconciled the open PR and fixed two shipping blockers: a safe-zone refusal now rolls back any just-created non-root model tab/pane instead of leaving a blank hostless surface, and failed shared WebView2 environment tasks are cleared by the awaiting caller after assignment (including synchronous UDF failures, without an old waiter erasing a newer retry). WebView2 now preserves address text typed during initialization and reports non-runtime startup failures accurately. Added controller rollback + Web-kind capacity regression coverage. Gates: cargo build clean; dotnet 262; cargo 25; app build 0W/0E; self-contained Release publish clean. Two focused read-only reviews found no residual P1/P2/P3 after fixes. Live GUI smoke remains pending: the Windows app-control helper timed out twice and reset, so PR #11 was deliberately left open rather than merged without the runbook §3a heavy-site/cap/UDF/clean-exit evidence.

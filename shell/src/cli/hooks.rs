...
fn parse_install(args: &[&str]) -> Result<CliInvocation, CliError> {
    ...
    let writes = vec![
        LocalFileWrite {
            relative_path: format!("{base}.ps1"),
            content: snippet(agent, "ps1"),
        },
        LocalFileWrite {
            relative_path: format!("{base}.cmd"),
            content: snippet(agent, "cmd"),
        },
    ];
...
fn parse_print(args: &[&str]) -> Result<CliInvocation, CliError> {
    ...
    Ok(CliInvocation {
        stdout: Some(snippet(agent, &format)),
        ..Default::default()
    })
}
...
pub fn snippet(agent: &AgentDef, format: &str) -> String {
    match format {
        "ps1" => [
            format!(
                "# optimus {0} hook (generated by `optimus hooks install {0}`)",
                agent.name
            ),
        ...
        "cmd" => [
            "@echo off".to_string(),
            format!(
                "rem optimus {0} hook (generated by `optimus hooks install {0}`)",
                agent.name
            ),
...
mod tests {
    ...
    fn install_emits_ps1_and_cmd_snippets() {
        let inv = parse_ok(
            &["hooks", "install", "codex", "--dir", r"C:\tmp\hooks"],
            &no_env,
            None,
        );
        ...
        assert!(writes
            .iter()
            .any(|w| w.relative_path == "optimus-codex-hook.ps1"));
        assert!(writes
            .iter()
            .any(|w| w.relative_path == "optimus-codex-hook.cmd"));
        assert!(inv.frames.is_empty());
    }
    ...
    fn print_outputs_snippet_to_stdout() {
        let inv = parse_ok(
            &["hooks", "print", "copilot", "--format", "cmd"],
            &no_env,
            None,
        );
        assert!(inv.frames.is_empty());
